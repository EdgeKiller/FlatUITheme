...
''' <summary>
''' Flat UI Theme
''' Creator: iSynthesis (HF) modified by EdgeKiller
''' Version: 1.0.4
''' Date Created: 17/06/2013
''' Date Changed: 02/07/2014
''' UID: 374648
...
Public Class FormSkin : Inherits ContainerControl
...
Public Class FlatClose : Inherits Control
...
Public Class FlatInfo : Inherits Control
...
#Region " Colors"
    ...
    Private _BaseColor As Color = Color.FromArgb(230, 126, 34)
    Private _TextColor As Color = Color.FromArgb(243, 243, 243)
...
#End Region
    ...
    Protected Overrides Sub OnPaint(e As PaintEventArgs)
        ...
        With G
            ...
            '-- X
            .DrawString("s", Font, New SolidBrush(TextColor), New Rectangle(0, 0, Width, Height), CenterSF)
...
Public Class FlatMax : Inherits Control
...
Public Class FlatMini : Inherits Control
...
Public Class FlatColorPalette : Inherits Control
...
#End Region
    ...
    Protected Overrides Sub OnPaint(e As PaintEventArgs)
        ...
        With G
            ...
            .FillRectangle(New SolidBrush(_Purple), New Rectangle(120, 0, 20, 40))
            .FillRectangle(New SolidBrush(_Gray), New Rectangle(100, 0, 20, 40))

            '-- Text
            .DrawString(Text, Font, New SolidBrush(Color.White), New Rectangle(-25, 22, W, H), CenterSF)
        End With
...
Public Class FlatGroupBox : Inherits ContainerControl
...
Public Class FlatButton : Inherits Control
...
<DefaultEvent("CheckedChanged")> Public Class FlatToggle : Inherits Control
...
<DefaultEvent("CheckedChanged")> Public Class RadioButton : Inherits Control
...
<DefaultEvent("CheckedChanged")> Public Class FlatCheckBox : Inherits Control
...
<DefaultEvent("TextChanged")> Public Class FlatTextBox : Inherits Control
...
Public Class FlatTabControl : Inherits TabControl
                    ...
                    If ImageList IsNot Nothing Then
                        Try
                            If ImageList.Images(TabPages(i).ImageIndex) IsNot Nothing Then
                                ...
                                '-- Text
                                .DrawString("      " & TabPages(i).Text, Font, New SolidBrush(TabPages(i).ForeColor), BaseSize, CenterSF)
                            Else
                                '-- Text
                                .DrawString(TabPages(i).Text, Font, New SolidBrush(TabPages(i).ForeColor), BaseSize, CenterSF)
                            End If
                    ...
                    If ImageList IsNot Nothing Then
                        Try
                            If ImageList.Images(TabPages(i).ImageIndex) IsNot Nothing Then
                                ...
                                .DrawImage(ImageList.Images(TabPages(i).ImageIndex), New Point(BaseSize.Location.X + 8, BaseSize.Location.Y + 6))
                                '-- Text
                                .DrawString("      " & TabPages(i).Text, Font, New SolidBrush(TabPages(i).ForeColor), BaseSize, New StringFormat With {.LineAlignment = StringAlignment.Center, .Alignment = StringAlignment.Center})
                            Else
                                '-- Text
                                .DrawString(TabPages(i).Text, Font, New SolidBrush(TabPages(i).ForeColor), BaseSize, New StringFormat With {.LineAlignment = StringAlignment.Center, .Alignment = StringAlignment.Center})
                            End If
...
Public Class FlatAlertBox : Inherits Control
...
Public Class FlatProgressBar : Inherits Control
...
Public Class FlatStickyButton : Inherits Control
...
Public Class FlatNumeric : Inherits Control

#Region " Variables"

    Private W, H As Integer
    Private State As MouseState = MouseState.None
    Private x, y, _scale As Integer
    Private _Value, _Min, _Max As Long
    Private Bool As Boolean
    Public Event ValueChange As EventHandler
...
#Region " Properties"
    ...
    Public Property Value As Long
        ...
        End Get
        Set(value As Long)
            If value <= _Max And value >= _Min Then
                _Value = value
                RaiseEvent ValueChange(Me, EventArgs.Empty)
            End If
            Invalidate()
        End Set
    End Property
    ...
    Public Property Scale As Integer
        ...
        Set(value As Integer)
            If value < 1 Then _scale = 1
            If value > 1 Then _scale = value
            Invalidate()
        End Set
    End Property
    ...
    Protected Overrides Sub OnMouseDown(e As MouseEventArgs)
        ...
        If x > Width - 21 AndAlso x < Width - 3 Then
            If y < 15 Then
                If (Value + _scale) <= _Max Then
                    _Value += _scale
                    RaiseEvent ValueChange(Me, EventArgs.Empty)
                End If

            Else
                If (Value - _scale) >= _Min Then
                    _Value -= _scale
                    RaiseEvent ValueChange(Me, EventArgs.Empty)
                End If
    ...
    Protected Sub OnValueChange(e As EventArgs)
...
Public Class FlatListBox : Inherits Control
...
#Region " Poperties"
    ...
    Sub Drawitem(ByVal sender As Object, ByVal e As DrawItemEventArgs) Handles ListBx.DrawItem
        ...
        If InStr(e.State.ToString, "Selected,") > 0 Then '-- if selected
            ...
            '-- Text
            e.Graphics.DrawString(" " & ListBx.Items(e.Index).ToString(), New Font("Segoe UI", 8), New SolidBrush(Color.White), e.Bounds.X, e.Bounds.Y + 2)
        Else
            '-- Base
    ...
    Function CountItem() As Integer
        Return ListBx.Items.Count
    End Function

    Sub SelectIndex(ByVal number As Integer)
        ListBx.SelectedIndex = number
    End Sub
...
Public Class FlatContextMenuStrip : Inherits ContextMenuStrip
    ...
    Public Class TColorTable : Inherits ProfessionalColorTable
...
<DefaultEvent("Scroll")> Public Class FlatTrackBar : Inherits Control
...
Public Class FlatStatusBar : Inherits Control
...
Public Class FlatLabel : Inherits Label
...
Public Class FlatTreeView : Inherits TreeView
